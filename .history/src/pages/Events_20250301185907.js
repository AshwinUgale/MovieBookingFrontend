...
const Events = () => {
  const [eventType, setEventType] = useState("music"); // Default to "music"
  const [events, setEvents] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // Fetch events when the eventType changes
  useEffect(() => {
    const getEvents = async () => {
      ...
      try {
        const data = await fetchEvents("New York", eventType.toLowerCase()); // Ensure lowercase
      ...
      <div className="filters mb-4 d-flex justify-content-center gap-3">
        <div>
          ...
          <select
            id="eventTypeSelect"
            value={eventType}
            onChange={(e) => setEventType(e.target.value.toLowerCase())} // Convert to lowercase
