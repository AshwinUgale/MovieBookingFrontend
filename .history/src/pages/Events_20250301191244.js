...
const Events = () => {
  const [eventType, setEventType] = useState(""); // Removed default "music"
  const [city, setCity] = useState("New York"); // Allow dynamic city selection
  const [events, setEvents] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // Fetch events when eventType or city changes
  useEffect(() => {
    const getEvents = async () => {
      ...
      try {
        const data = await fetchEvents(city, eventType?.toLowerCase() || undefined); // Avoid forcing "music"
    ...
    <div className="container mt-5">
      ...
      <div className="filters mb-4 d-flex justify-content-center gap-3">
        ...
        <div>
          ...
          >
            <option value="">All Events</option> {/* Allow all categories */}
