...
const Events = () => {
  // Only maintain filter state for event type
  const [eventType, setEventType] = useState("music");
  const [events, setEvents] = useState([]);
  const [loading, setLoading] = useState(true);

  // Fetch events whenever the eventType filter changes
  useEffect(() => {
    const getEvents = async () => {
      setLoading(true);
      try {
        // Pass the default city ("New York") and the current eventType filter
